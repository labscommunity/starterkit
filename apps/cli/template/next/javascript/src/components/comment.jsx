...
export function CommentDialog(props) {
  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      console.log("Added comment successfully for:", props.txId, result.transaction);
  ...
  function trimAddress(str, startLength = 4, endLength = 3) {
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <ScrollArea className="h-36 rounded-md border p-4">
          {isCommentsLoading ? (
            ...
            <div className="flex justify-center items-center h-full">No comments yet!</div>
          )}
        </ScrollArea>

        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 border-t">
            <FormField
            ...
            />
            <DialogFooter>
              <Button type="submit">{isLoading ? <Spinner /> : "Comment"}</Button>
            </DialogFooter>
            <p className="pt-2 text-xs text-muted-foreground">Comments may take sometime to load.</p>
          </form>
        </Form>
      </DialogContent>
    </Dialog>
